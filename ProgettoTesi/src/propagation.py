...
def propagation(sentences, local_corenlp_path): # questo metodo prende in input la lista di frasi e la libreria
    ...
    for input in sentences: # per ogni singola frase nella lista di frasi
        ...
        with open('../target.txt', 'a') as file_tar_r1: # apre il file dei target inizialmente vuoto in append
            ...
            print("REGOLA 1")
            print('\n')
            tar_r1 = rule.R1(local_corenlp_path, input) # estrapolazione del target tramite la regola R1, passando
            ...
            for word_r1 in tar_r1:    # targ è la lista di tutti i target estratti dalla singola frase (ovvero input)
                file_tar_r1.write('\n') # scrive uno spazio nel file dopo #
        ...
        with open('../opinion.txt', 'a') as file_opi_r2:
            ...
            print("REGOLA 2")
            print('\n')
            opi_r2 = rule.R2(local_corenlp_path, input, target)# estrapolazione della parola di opinione tramite la regola R2, passando
        ...
        with open('../target.txt', 'a') as file_tar_r3:  # apre il file dei target inizialmente vuoto in append
            ...
            print("REGOLA 3")
            print('\n')
            tar_r3 = rule.R3(local_corenlp_path, input, targe)# estrapolazione del target tramite la regola R3, passando
        ...
        with open('../opinion.txt', 'a') as file_opi_r4:
            ...
            print("REGOLA 4")
            print('\n')
            opi_r4 = rule.R4(local_corenlp_path, input, opin) # estrapolazione della parola di opinione tramite la regola R4, passando
                                                        # al metodo la libreria, la frase da esaminare e il file dei target singoli estratti prima
